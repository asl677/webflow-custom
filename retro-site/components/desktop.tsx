...
export interface OpenWindow {
  ...
  type:
    ...
    | "word-editor" // Added word-editor type
    | "add-project" // Added add-project type
  // Removed "duke-nukem" from OpenWindow type
  title: string
...
export function Desktop() {
  ...
  const calculateIconPositions = useCallback(() => {
    if (typeof window !== "undefined") {
      ...
      // Icons that should be on the left side
      const leftIcons = [
        "my-computer",
        "gta-game", 
        "doom-game",
      ...
      // Icons that should be pinned to the right side
      const rightIcons = [
        "registry",
        "factory", 
        "blog",
      ...
      // Position left side icons in 2-column grid
      leftIcons.forEach((name, i) => {
        const col = i % cols
      ...
      // Position right side icons in 2-column grid, pinned to the right
      const rightStartX = window.innerWidth - (iconSize * 2 + gap) - 20 // 20px margin from right edge
      rightIcons.forEach((name, i) => {
        const col = i % cols
        const row = Math.floor(i / cols)
        positions[name] = {
          x: rightStartX + col * (iconSize + gap),
          y: startY + row * (iconSize + gap),
        }
      })
    ...
    // Fallback for SSR - left side icons in 2 column grid, right side icons in 2 column grid
    const iconSize = 48
    const gap = 15
    const cols = 2
    const startX = 20
    ...
    leftIcons.forEach((name, i) => {
      const col = i % cols
      const row = Math.floor(i / cols)
      defaultIconPositions[name] = {
        x: startX + col * (iconSize + gap),
        y: startY + row * (iconSize + gap),
      }
    })

    // Position right side icons in 2-column grid (using 1200px as fallback width)
    const rightStartX = 1200 - (iconSize * 2 + gap) - 20 // 20px margin from right edge
    rightIcons.forEach((name, i) => {
      const col = i % cols
      const row = Math.floor(i / cols)
      defaultIconPositions[name] = {
  ...
  const openFolder = (iconType: string, title: string) => {
    ...
    const iconMap: { [key: string]: string } = {
      ...
      "recycle-bin":
        "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/recycle_bin_full-4-IdP8ANgtkYeB1LXuBgnXy9kEhtzG55.png",
      registry: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/cable_2-0-Rv4vJ8EwL56R4eJL4PIyU9N9sa8rp4.png",
      apps: "https://blob.v0.app/app-icon.png",
      resources: "/icons/resources-folder.png",
      "why-514": "/icons/notepad-icon.png",
    }
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      icon:
        iconMap[iconType] ||
        "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/directory_closed-4-cw7WV21hUuNvjuYfejy4X7LimHTLlf.png",
    }
  ...
  const openDocument = (docTitle: string, content: string) => {
    ...
    if (docTitle === "case-study-viewer") {
      ...
      const windowData: Omit<OpenWindow, "isAnimating"> = {
        ...
        type: "case-study-viewer",
        title: "Case Study",
        content: { caseStudyId: content },
        position: getWindowPosition(),
        zIndex: nextZIndex,
    ...
    if (docTitle === "testimonial-viewer") {
      ...
      const windowData: Omit<OpenWindow, "isAnimating"> = {
        ...
        type: "testimonial-viewer",
        title: "Testimonial",
        content: { testimonialId: content },
        position: getWindowPosition(),
        zIndex: nextZIndex,
        size: { width: 700, height: 500 },
      }
    ...
    if (docTitle === "white-paper-viewer") {
      ...
      const windowData: Omit<OpenWindow, "isAnimating"> = {
        ...
        type: "white-paper-viewer",
        title: "AIOps Network Operations.pdf",
        content: { content },
        position: getWindowPosition(700, 600),
        zIndex: nextZIndex,
  ...
  const openApps = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      isMinimized: false,
      content: {},
      defaultTab: "apps", // Set default tab to apps
      icon: "/icons/notepad.png",
    }
  ...
  const openRegistry = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      isMinimized: false,
      content: {},
      defaultTab: "connectors", // Set default tab to connectors for registry
  ...
  const openCaseStudies = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `case-studies-${Date.now()}`,
      type: "folder",
      title: "Case Studies",
      content: { iconType: "case-studies" },
      position: getWindowPosition(),
      zIndex: nextZIndex,
  ...
  const openTestimonials = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      type: "folder",
      title: "Clients", // Changed from "Testimonials" to "Clients"
      content: { iconType: "testimonials" },
      position: getWindowPosition(),
      zIndex: nextZIndex,
      size: { width: 600, height: 400 },
      icon: "/icons/notepad.png",
  ...
  const openODW = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `blog-odw-${Date.now()}`,
      type: "blog-post-viewer",
      title: "Operational Data Warehouse - Blog Posts",
      position: getWindowPosition(),
      size: { width: 700, height: 600 },
      zIndex: getNextZIndex(),
      isMinimized: false,
      content: { initialPost: "data-intensive" },
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openUFA = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `blog-ufa-${Date.now()}`,
      type: "blog-post-viewer",
      title: "User Facing Analytics - Blog Posts",
      position: getWindowPosition(),
      size: { width: 700, height: 600 },
      zIndex: getNextZIndex(),
      isMinimized: false,
      content: { initialPost: "user-analytics" },
    }
    createAnimatedWindow(windowData)
  }
  ...
  const handleStartMenuClick = (item: string) => {
    ...
    switch (item) {
      ...
      case "Dark Mode":
        ...
        setIsStartMenuOpen(false) // Close start menu when toggling dark mode
        break
      case "Company":
        openCompany()
        break
  ...
  const openInternetExplorer = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      position: getWindowPosition(500, 390),
      zIndex: nextZIndex,
      size: { width: 500, height: 390 },
      icon: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/msie1-2-9mEBMDH09EZhzEX53E25B4rvxZS7RD.png",
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openTemplateViewer = (initialTemplate = "analytics") => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `template-viewer-${Date.now()}`,
      type: "template-viewer",
      ...
      icon: "/icons/notepad.png", // Added icon
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openDemoRequest = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/envelope-closed.png", // Fixed icon from notepad to envelope
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openCursorEditor = (templateName: string) => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `cursor-${templateName}-${Date.now()}`,
      type: "cursor-editor",
      title: `Cursor - ${templateName} Template`,
      content: { templateName },
      position: getWindowPosition(),
      zIndex: nextZIndex,
      size: { width: 700, height: 400 },
      icon: "/icons/mouse_speed.png", // Added cursor icon
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openCursorWithData = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `cursor-data-${Date.now()}`,
      type: "cursor-editor",
      title: "Cursor - Data.json",
      content: { templateName: "data-json" },
      position: getWindowPosition(),
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/notepad.png",
  ...
  const openSloanAI = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/color-profile-icon.png", // Fixed icon for Sloan AI
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openNotepad = (title?: string, content?: string) => {
    ...
    createAnimatedWindow({
      id: "notepad",
      type: "notepad",
      title: title || "why-514.txt",
      icon: "/icons/notepad-icon.png",
      content: { title, initialContent: content },
      position: {
  ...
  const openDocs = () => {
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      type: "docs",
      title: "Documentation",
      position: getWindowPosition(600, 500),
      size: { width: 600, height: 500 },
      zIndex: nextZIndex,
      isMinimized: false,
      icon: "/icons/notepad.png",
  ...
  const openSchemaWindow = (item: any, activeTab = "schema") => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `schema-${item.name}-${Date.now()}`,
      type: "schema-window",
      title: `${item.name} - Schema`,
      position: { x: centerX, y: centerY },
      zIndex: nextZIndex,
      size: { width: windowWidth, height: windowHeight },
      item,
  ...
  const openAddProject = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `add-project-${Date.now()}`,
      type: "add-project",
      title: "Add New Project",
      position: getWindowPosition(400, 200),
      zIndex: nextZIndex,
      size: { width: 400, height: 200 },
      icon: "/icons/diskettes-icon.png", // Fixed icon for add project
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openUFADownloadWindow = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `ufa-download-${Date.now()}`,
      type: "ufa-download-window",
  ...
  const openODWDownloadWindow = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `odw-download-${Date.now()}`,
      type: "odw-download-window",
  ...
  const openPricingPage = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `pricing-${Date.now()}`,
      type: "pricing-page",
      title: "Pricing - Boreal",
      position: getWindowPosition(500, 400),
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/pricing-icon.png",
    }
  ...
  const openCompany = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `ie-company-${Date.now()}`,
      type: "internet-explorer",
      title: "Internet Explorer - Company",
      position: getWindowPosition(),
      zIndex: nextZIndex,
      size: { width: 600, height: 500 },
      initialTab: "company",
  ...
  const openDataman = () => {
    ...
    const newWindow: OpenWindow = {
      ...
      title: "Data-Man",
      position: {
        x: isMobile ? 20 : Math.max(50, (screenWidth - windowWidth) / 2),
        y: isMobile ? 20 : Math.max(50, (screenHeight - windowHeight) / 2),
      },
      zIndex: nextZIndex,
      size: { width: windowWidth, height: windowHeight },
      icon: "/icons/color-profile-icon.png", // Fixed icon for dataman game
    }
  ...
  const handleIconDoubleClick = (iconId: string) => {
    ...
    switch (iconId) {
      ...
      case "docs":
        ...
        break
      case "why-514":
        openNotepad("why-514.txt", "Why 514? This is a placeholder text file.")
        break
      case "white-papers":
        openWhitePapers()
        break
  ...
  const renderWindowContent = (window: OpenWindow) => {
    ...
    switch (window.type) {
      case "ufa-download-window":
        return (
          <UFADownloadWindow
            ...
            icon={window.icon} // Pass the icon property
          />
        )
      ...
      case "doom-game": // Replaced simcity-game rendering
        return (
          <DoomGame
            key={window.id}
            position={window.position}
  ...
  const openDatamanGame = () => {
    ...
    const newWindow: OpenWindow = {
      ...
      zIndex: nextZIndex,
      size: { width: windowWidth, height: windowHeight },
      icon: "/icons/color-profile-icon.png", // Fixed icon
    }
  ...
  const openWindow = (
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      position,
      zIndex: nextZIndex,
      size: size || getWindowPosition(400, 320), // Default size if not provided
      content,
      initialTab,
      defaultTab,
      connectorName,
  ...
  const openCursorEditorWindow = (templateName: string) => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      title: `Cursor - ${templateName} Template`,
      content: { templateName },
      position: getWindowPosition(700, 400),
      zIndex: nextZIndex,
      size: { width: 700, height: 400 },
    }
    createAnimatedWindow(windowData)
  ...
  const openAddressBook = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `address-book-${Date.now()}`,
      type: "folder",
      title: "Address Book",
      content: { iconType: "address-book" },
      position: getWindowPosition(500, 400),
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/address-book-users.png", // Fixed icon
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openAddressBookUsers = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      id: `address-book-users-${Date.now()}`,
      type: "folder",
      title: "Address Book Users",
      content: { iconType: "address-book-users" },
      position: getWindowPosition(500, 400),
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/address-book-users.png", // Fixed icon
  ...
  const openBlogPostViewer = () => {
    ...
    const windowData: Omit<OpenWindow, "isAnimating"> = {
      ...
      zIndex: nextZIndex,
      size: { width: 500, height: 400 },
      icon: "/icons/blog-icon.png", // Fixed icon
    }
    createAnimatedWindow(windowData)
  }
  ...
  const openPacman = () => {
    ...
    const newWindow: OpenWindow = {
      id: `pacman-${Date.now()}`,
      type: "folder", // Assuming pacman is a folder for now, adjust if it's a specific game type
      title: "Pac-Man",
      position,
      ...
      isAnimating: true,
      isMinimized: false,
      content: { iconType: "pacman" }, // Use iconType to identify pacman folder
      icon: "/icons/notepad.png",
    }
    setOpenWindowsState((prev) => [...prev, newWindow])
    setTimeout(() => {
  ...
  return (
    <div
      className={`relative w-full h-dvh ${isDarkMode ? "overflow-auto" : "overflow-hidden"} select-none ${isDarkMode ? "dark" : ""}`}
      style={{ backgroundColor: isDarkMode ? "#000000" : "rgb(0, 128, 128)", height: "100vh", overflow: "hidden" }}
      onClick={handleDesktopClick}
    >
      <DesktopIcon
        id="my-computer"
        icon="/icons/computer-explorer-cool.png" // Updated to use new computer icon
        label="My Computer"
      ...
      <DesktopIcon
        id="recycle-bin"
        icon="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/recycle_bin_full-4-IdP8ANgtkYeB1LXuBgnXy9kEhtzG55.png"
        label="Recycle Bin"
      ...
      <DesktopIcon
        ...
        onSelect={handleIconSelect}
        onDoubleClick={() => handleIconDoubleClick("figma")}
      ...
      <DesktopIcon
        ...
        icon="/icons/resources-folder.png"
      ...
      <DesktopIcon
        ...
        label="why-514.txt"
        icon="/icons/notepad-icon.png"
        position={iconPositions["why-514"]}
        onMove={(pos) => updateIconPosition("why-514", pos)}
        onDoubleClick={() => openNotepad("why-514.txt", "Why 514? This is a placeholder text file.")}
        isSelected={selectedIcon === "why-514"}
        onSelect={() => setSelectedIcon("why-514")}
        isDarkMode={isDarkMode}
      />
      ...
      {!isMobile && (
        <>
          <DesktopIcon
            ...
            icon="/icons/doom.png"
            label="DOOM"
            position={iconPositions["doom-game"]}
            isSelected={selectedIcon === "doom-game"}
            onSelect={handleIconSelect}
            onDoubleClick={() => handleIconDoubleClick("doom-game")}
            onMove={(pos) => updateIconPosition("doom-game", pos)}
            isDarkMode={isDarkMode}
          />
          <DesktopIcon
            id="simcity-game"
      ...
      <DesktopIcon
        id="request-demo"
        icon="/icons/envelope-closed.png" // Updated to use envelope icon for Request Demo
